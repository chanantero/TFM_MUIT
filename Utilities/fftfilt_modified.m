function y = fftfilt_modified(b, x, nfft)
% Modification of fftfilt by Rubén Chuliá on 20/05/2018, in order to serve
...
narginchk(2,3); % Throws an error if the number of input arguments is less than 2 or greater than 3

numSignals = size(x, 2);
numFilters = size(b, 2);
numOutputSignals = size(b, 3);

nx = size(x,1);

if (numFilters ~= numSignals) && (numSignals > 1) && (numFilters > 1)
