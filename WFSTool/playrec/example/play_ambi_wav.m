...
%Test if current initialisation is ok 如果已经ini了，则检查，否则跳过此段
...
if(~playrec('isInitialised'))
    ...
    % -1 represents that no record device selected
...
for startSample = startPoint:pageSize:endPoint
    ...
    % 每次循环将新用到的pagenum接在之前的page之后，形成pagelist
    % replace bformat_dec by WFS???
    ...
    if(length(pageNumList) > pageBufCount) % 超出buffer可以播放了
        if(runMaxSpeed) % same function but different time cost
            while(playrec('isFinished', pageNumList(1)) == 0)
            % 检查pagelist的第一个是不是播放完了
            end
        else        
        ...
        % delete the first page num of the list
    end
end
