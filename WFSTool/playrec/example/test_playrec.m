...
if exist('playrec', 'file') ~= 3 % 3 means name exists as a MEX-file on your MATLAB search path.
...
% find the sentence 'Built with...' in the playrex('about')
% regexp+i, i means case insensitive 大小写不敏感
...
if ~isempty(deviceSummary)
    ...
    % cell array to array (cell array：矩阵内分成块)  
end
...
if playDevId == -1
    ...
    for rate = samplerates
        try
            playrec('init', rate, playDevId, -1);  % Initialise the device
    ...
    if ~playrec('isInitialised')
        ...
        for loopcount=0:49
            ...
            sample_no = loopcount * rate/10 + (1:rate/10)';   % ' means 转置 rate should be 192000 for earphone
