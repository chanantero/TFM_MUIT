...
% Edited by Rubén Chuliá (rculia@outlook.com) so it can work well for
% arrows in the z direction. Assume that the length in pixels of the z
% dimension of the screan is the mean of the other two (x and y)
	D = sqrt(sum(((Xf(1:3,:)-X0(1:3,:)).*([ap; mean(ap)]./[limrange; 1])).^2)); % Original line: D = sqrt(sum(((Xf(1:3,:)-X0(1:3,:)).*(ap./limrange)).^2));
...
%	compute and modify along-arrow distances
	len1 = len;
	len2 = len - (len.*tand(tipangle)-wid/2).*tand(90-baseangle);
